...
class MyGenerator(nn.Module):
    ...
    def forward(self, x):  # 100*1*1
        ...
        out = self.block4(out)  # 64*1*32*32
        out = self.layer5(out)  # 3*96*96
        return self.tanh(out)  # 限制像素值

    def weight_init(m):  # 优化: 权重初始化
...
class MyDiscriminator(nn.Module):
    def __init__(self):
        ...
        self.cnn5 = nn.Conv2d(dis_feature * 8, 1, kernel_size=4, stride=1, padding=0, bias=False)  # 64*8*4*4 -> 1
        #  优化: 改成WGAN-gp故删去Sigmoid层
...
def calculate_gp(img, fake_img):
    ...
    eta = eta.expand_as(img).to(device)

    # 插值得到新图像
    interplt_img = torch.add(torch.mul(eta, img), torch.mul(1 - eta, fake_img))
...
def train(dataset, batch_size, noise_size, gen_epoch, dis_epoch):
    sum_loss_D = 0
    sum_loss_G = 0
    p_one = torch.tensor(1, dtype=torch.float).to(device)
    m_one = (p_one * -1).to(device)
    ...
    for i, img in tqdm.tqdm(enumerate(train_dataloader)):  # 256 * 3 * 96 * 96
        ...
        if (i + 1) % dis_epoch == 0:
            ...
            gradient_penalty = calculate_gp(img, dis_fake_img)
            gradient_penalty.backward()

            dis_optimizer.step()

            sum_loss_D += (discriminator_fake_loss - dis_true_loss + gradient_penalty).item()
        ...
        if (i + 1) % gen_epoch == 0:
            ...
            gen_noises = torch.randn(batch_size, noise_size, 1, 1)
            gen_noises = gen_noises.to(device)

            gen_fake_img = Generator(gen_noises)
            gen_fake_output = Discriminator(gen_fake_img)

            generator_loss = gen_fake_output.mean()
            generator_loss.backward(m_one)
...
def show(num, batch_size, noise_size):
    ...
    plt.suptitle('w_epoch {}'.format(num), fontsize=15)
    plt.savefig('w_epoch {}'.format(num) + '.png')


train_dataset = MyDataset(path_list, transform)
plot_epoch = [49, 99, 149, 199, 249, 299, 349, 399, 499]
lossD_list = []
lossG_list = []
for i in range(sum_epochs):
    ...
    print('w_epoch {}: loss of D {} , loss of G {}'.format(i, lossD, lossG))
    if i in plot_epoch:
        show(i + 1, batch_size, noise_size)

torch.save(Discriminator, 'w_Dnet.pkl')
torch.save(Generator, 'w_Gnet.pkl')
...
plt.grid(True)
plt.legend()
plt.savefig('w_lossD_vs_lossG.png')